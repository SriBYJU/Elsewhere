export type WorldKind = 'manhattan' | 'neural' | 'semiconductor' | 'system';
export type SystemArchetype = 'company' | 'finance' | 'supply-chain' | 'cpu' | 'internet' | 'science' | 'history' | 'energy' | 'ecosystem' | 'general';
...
export interface Variable {id:string; label:string; min:number; max:number; step:number; initial:number; unit:string; description:string; claimIds:string[];targetId?:string}
...
export interface BlueprintNode {id:string;label:string;role:string;description:string;baseline:number;position:[number,number,number];sourceIds:string[]}
export interface BlueprintEdge {from:string;to:string;polarity:-1|1;strength:number;lag:number;confidence:'high'|'medium'|'low';description:string;sourceIds:string[]}
export interface SystemBlueprint {archetype:SystemArchetype;subject:string;summary:string;nodes:BlueprintNode[];edges:BlueprintEdge[];variables:Variable[];sources:Source[];researchStatus:'researched'|'offline';limitations:string[]}
...
export interface WorldDocument {format:'elsewhere';version:1;id:string;kind:WorldKind;title:string;question:string;createdAt:string;updatedAt:string;parameters:Parameters;time:number;branches:Branch[];activeBranchId:string;history:{at:string;action:string}[];blueprint?:SystemBlueprint}
